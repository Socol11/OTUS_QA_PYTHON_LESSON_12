...
def test_login_admin_page(browser, url):
    ...
    wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, "input[id='input-username']"))).send_keys('user')
    wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, "input[id='input-password']"))).send_keys('bitnami')
...
def test_unlogin_admin_page(browser, url):
    browser.get(url + '/admin')
    wait = WebDriverWait(browser, 5)
    wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, "input[id='input-username']"))).send_keys('user')
    wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, "input[id='input-password']"))).send_keys('bitnami')
    ...
    try:
        ...
        raise AssertionError(f"Выход из панели управления не удался!")
